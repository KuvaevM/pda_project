...
def test_automat_demo(automat, input_file: str) -> None:
    output_file = input_file.replace("in", "out")
    pointer = 1
    f_input = open(input_file, "r")
    f_output = open(output_file, "w")
    input_str = f_input.readline().replace('\n', '')
    if input_str == "":
        cmd = f_input.readline().replace('\n', '')

        while cmd.split()[0] != "next":
            print("Wrong command! Please, try again.", file=f_output)
            cmd = f_input.readline().replace('\n', '')
        flag = False
        for rule in automat.rules:
            if rule.left.state == automat.states[0] and rule.left.input_symbol.output() == "Empty" and \
                ...
                print("Automate successfully ended. Step sequence:", file=f_output)
                print(f"({automat.states[0]}, Empty, {automat.start_stack_element}){SEPARATOR}({automat.states[0]},"
                      f" Empty, Empty)", file=f_output)
                flag = True
                break
        if not flag:
            print("String is not accepted by automat. All sequences:", file=f_output)
            print(f"({automat.states[0]}, Empty, {automat.start_stack_element})", file=f_output)
    else:
        ...
        while len(current_seq.current_steps) != 0:
            ...
            if len(cmd.split()) == 2 and cmd.split()[0] == "next" and cmd.split()[1].isdigit():
                ...
                for i in range(int(cmd.split()[1]) - 1):
                    current_seq = StepsSeq(test_emulate_automate(current_seq, automat, f_output))
                    if len(current_seq.current_steps) == 0:
                        is_empty=True
                        break
                if is_empty:
                    break
                current_seq = test_print_next(current_seq, automat, f_output)
            elif cmd == "next":
                current_seq = test_print_next(current_seq, automat, f_output)
            else:
                print(cmd)
...
def test_print_next(current_seq, automat, f_output) -> StepsSeq:
    new_seq_steps = test_emulate_automate(current_seq, automat, f_output)
    if success:
        return StepsSeq([])
    for step in new_seq_steps:
        print(step.previous_steps, file=f_output)
    return StepsSeq(new_seq_steps)


def test_emulate_automate(current_seq, automat, f_output) -> List[Step]:
    new_seq_steps = []
    for step in current_seq.current_steps:
        ...
        for rule in suitable_rules:
            ...
            if len(next_stack) == 0 and len(next_end_str) == 0:
                ...
                success = True
                print("Automate successfully ended. Step sequence:", file=f_output)
                print(step.previous_steps + SEPARATOR + f"({step.current_state}, Empty, Empty)", file=f_output)
                return []
            elif len(next_stack) == 0:
                unsuccessfull_secs.append(step.previous_steps + SEPARATOR + f"({step.current_state}, {next_end_str}, "
                                                                            f"Empty)")
                print(unsuccessfull_secs[-1], file=f_output)
                continue
            elif next_end_str == "":
